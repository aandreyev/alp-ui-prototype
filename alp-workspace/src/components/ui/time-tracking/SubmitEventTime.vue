<template>
  ...
  >
    <Tabs :default-value="state.selectedTab" v-model="state.selectedTab">
      <div
        class="mx-auto w-10 flex items-center justify-center timeEntriesTimer"
      >
        <TabsList>
          <TabsTrigger
            value="sales"
...
watch(
  ...
  ([newUnits, newDate, newDescription, newEventId]) => {
...
function updateModalDescription(newTab: string) {
  switch (newTab) {
    case "sales":
      state.modalTitle = "Record Sales Time over Event";
      ...
      break;
    case "matter":
      state.modalTitle = "Record Matter Time over Event";
      state.modalDescription =
      ...
      break;
    case "project":
      state.modalTitle = "Record Project Time over Event";
      state.modalDescription =
