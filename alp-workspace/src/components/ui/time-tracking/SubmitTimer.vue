  ...
  >
    <Tabs :default-value="state.selectedTab" v-model="state.selectedTab">
      <div class="mx-auto w-10 flex items-center justify-center">
        <TabsList>
          <TabsTrigger value="sales" v-if="props.timer.type === TimerType.Free">
            Sales
          </TabsTrigger>
          <TabsTrigger
            value="matter"
            v-if="
              timer.type == TimerType.MatterComponent ||
              timer.type == TimerType.Free
            "
          >
            Matter
          </TabsTrigger>
          <TabsTrigger
            value="project"
            v-if="
              timer.type == TimerType.ProjectTask ||
              timer.type == TimerType.Free
            "
          >
            Project
          ...
          <shad-form-control>
            <text-area
              ...
              @update:modelValue="(val: string) => {
                handleSubmit.setFieldValue('description', val);
...
import { TimeEntryTypes, BillableTypes, GstTypes, TimerType } from "@/network/dtos/enumTypes";
import { MatterDto } from "@/network/dtos/matter-dto";
import { TimeEntryInput } from "@/network/dtos/time-entry-dto";
import { TimerDto } from "@/network/dtos/timer-dto";
...
const componentSchemaWithMatter = {
  ...baseSchema,
  rate: z.number({ required_error: "Rate is required" }).optional()
};
  ...
  switch (state.selectedTab) {
    ...
    case "matter":
      if (props.componentId != null) {
        return toTypedSchema(z.object(componentSchemaWithMatter));
      } else {
...
const updatedTimer = computed(() =>
  ApiStore.toData<TimerDto>(
    store.getters[TimeEntryStore.getters.GET_TIMER](props.timer.id)
  )
);
...
const submit = handleSubmit.handleSubmit((values) => {
  ...
  if (entityId != null) {
    ...
    if (props.timer?.id) {
      store
        .dispatch(TimeEntryStore.actions.STOP_TIMER, {
          id: props.timer?.id,
...
onMounted(() => {
  ...
  getBillingRate();
  state.date = new Date();
  state.units = updatedTimer.value?.accumulatedUnits ?? 0;
  state.description = props.timer?.description ?? "";
  if (state.description) {
    handleSubmit.setFieldValue("description", state.description);
    handleSubmit.resetForm({ values: { description: state.description } });
  }
});

onMounted(() => {
  store.dispatch(TimeEntryStore.actions.GET_TIMER, { id: props.timer.id });
});
...
function updateModalDescription(newTab: string) {
  switch (newTab) {
    case "sales":
      state.modalTitle = "Record Sales Time over Timer";
      ...
      break;
    case "matter":
      state.modalTitle = "Record Matter Time over Timer";
      state.modalDescription =
      ...
      break;
    case "project":
      state.modalTitle = "Record Project Time over Timer";
      state.modalDescription =
...
watch(
  () => props.timer,
  () => {
    const description = props.timer?.description ?? "";
    state.description = description;
    handleSubmit.setFieldValue("description", description);
    state.date = new Date();
  },
  { immediate: true }
);
...
watch(
  ...
  () => {
    state.units = updatedTimer.value?.accumulatedUnits ?? 0;
    handleSubmit.setFieldValue("units", state.units);
    // console.log("accumulatedTime",updatedTimer.value?.accumulatedTime );
  },
  { immediate: true, deep: true }
);
