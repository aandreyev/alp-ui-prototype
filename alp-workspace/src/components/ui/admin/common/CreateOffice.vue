<template>
  <modal @close="$emit('close')" :headingTitle ="'Create Office'">
    <modal-form @cancel="$emit('close')" @submit="createOffice">
      <alp-form-container>
        ...
        <field-label class="w-full" name="Phone">
          <v-field
            type="text"
            placeholder="Phone"
            name="phone"
        ...
        <field-label class="w-full" name="Website" :isRequired="true">
          <v-field
            type="text"
            placeholder="Website"
            name="website"
            rules="required"
          />
          <error-message class="error-message" name="website" />
        </field-label>
...
import { OfficeInput } from "@/network/dtos/office-dto";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function createOffice(values: OfficeInput) {
      store
        .dispatch(CommonStore.actions.CREATE_OFFICE, OfficeInput.fromJS(values))
        .then(() => {
          emit("close");
        });
    }
