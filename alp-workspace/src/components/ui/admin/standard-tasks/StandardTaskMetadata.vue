...
import { StandardTaskServiceProxy } from "@/network/projects-service-proxies";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function fetchDocumentParameters() {
      new StandardTaskServiceProxy()
        .getStandardTaskMetadataValues(parseInt(props.id))
        .then((result) => {
    ...
    function removeDynamicValue(id: number) {
      new StandardTaskServiceProxy()
        .removeStandardTaskMetadataValue(parseInt(props.id), id)
        .then(fetchDocumentParameters);
    }
