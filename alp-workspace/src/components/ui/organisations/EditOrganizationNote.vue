  ...
  <modal @close="$emit('close')" :headingTitle="'Update Note'">
    <div class="flex flex-col rounded-lg bg-white min-h-0">
      <div class="flex flex-col pb-3 px-6">
        ...
        <span
          class="flex flex-wrap gap-2 flex-wrap gap-2 justify-end space-x-2 py-2"
        >
...
import { OrganisationNoteServiceProxy } from "@/network/organisation-service-proxies";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      new OrganisationNoteServiceProxy()
        .getOrganisationNoteById(props.id, props.noteid)
        .then((result: NoteDto) => {
          state.note = result;
        });
    });
    ...
    function updateNote() {
      if (state.note.message) {
        if (state.fileName.length > 0) {
          ...
          store
            .dispatch(
              OrganisationStore.actions.ADDUPDATE_ORGANIZATION_NOTE_UPLOAD,
              {
                id: props.id,
          ...
          store
            .dispatch(OrganisationStore.actions.ADDUPDATE_ORGANIZATION_NOTE, {
              id: props.id,
              attachments: state.removeAttachmentId,
              note: new NoteInput({
                message: state.note.message,
