<template>
  <modal @close="$emit('close')" :headingTitle="'Withdrawal Funds in Trust'">
    <modal-form
      @cancel="$emit('close')"
    ...
    >
      <alp-form-container>
        ...
        <field-label
          ...
          name="Trust Account"
          :isRequired="true"
          v-if="isControlledMoneyAvailable"
        >
          <available-trust-account-for-matter-selector
            v-model="state.trustAccount"
            placeholder="Trust Account"
            rules="required"
            :matterId="id"
          />
          <error-message class="error-message" name="trustAccount" />
        </field-label>

        <!-- Transfer -->
        <template v-if="values.transactionType == TransactionType.TransferOut">
          <field-label name="Transfer To Matter" class="w-full" :isRequired="true">
        ...
        <template v-if="values.transactionType == TransactionType.Deposit">
          <field-label class="w-1/2" name="Transaction Method" :isRequired="true">
          ...
          <template v-if="values.transactionMethod == TransactionMethod.Cheque">
            <field-label class="w-full sm:w-1/2" name="Cheque Name" :isRequired="true">
              <v-field
                type="text"
            ...
            <field-label class="w-full sm:w-1/2" name="Cheque Bank" :isRequired="true">
              <v-field
                type="text"
            ...
            <field-label class="w-full sm:w-1/2" name="Cheque BSB" :isRequired="true">
              <v-field
                type="text"
            ...
            <field-label class="w-full sm:w-1/2" name="Cheque Number" :isRequired="true">
              <v-field
                type="text"
          ...
          <template v-if="values.depositPurpose == DepositPurpose.InvoicePayment">
            <field-label class="w-1/2" name="Invoice Number">
              <v-field type="number" placeholder="Invoice Number" name="invoiceNumber" />
            </field-label>
          </template>
          ...
          <field-label class="w-1/2" name="Depositor Address" :isRequired="true">
            <v-field
              type="text"
        ...
        <template v-if="values.transactionType == TransactionType.Withdrawal">
          ...
          <field-label class="w-1/2" name="Payment Direction" :isRequired="true">
            <v-field as="select" name="withdrawalPaymentDirection" rules="required">
            ...
            <error-message class="error-message" name="withdrawalPaymentDirection" />
          </field-label>

          <field-label class="w-1/2" name="Withdrawal Purpose" :isRequired="true">
          ...
          <template v-if="values.withdrawalPurpose == WithdrawalPurpose.InvoicePayment">
          ...
          <template v-if="values.transactionMethod == TransactionMethod.EFT">
            ...
            <field-label class="w-1/2" name="Payee Account Number" :isRequired="true">
              <v-field
                type="text"
                ...
                rules="required"
              />
              <error-message class="error-message" name="withdrawalPayeeAccountNumber" />
            </field-label>
            <field-label class="w-1/2" name="Payee EFT Reference" :isRequired="true">
              <v-field
                type="text"
                ...
                rules="required"
              />
              <error-message class="error-message" name="withdrawalEftReference" />
            </field-label>
          </template>
          <template v-if="values.transactionMethod == TransactionMethod.Cheque">
            <field-label class="w-1/2" name="Cheque Reference" :isRequired="true">
              <v-field
                type="text"
                ...
                rules="required"
              />
              <error-message class="error-message" name="withdrawalChequeReference" />
            </field-label>
          </template>
        </template>
        ...
        <field-label class="w-full sm:w-1/2" name="Transaction Date" :isRequired="true">
          <date-field
        ...
        <field-label class="w-full sm:w-1/2" name="Amount" :isRequired="true">
          <v-field placeholder="Amount" name="amount" type="number" rules="required" />
            ...
            <div class="w-11/12 flex-wrap justify-between space-x-2">
              <div class="grid grid-cols-2 gap-2">
                ...
                <v-field
                  ...
                  rules="url"
                />
                <error-message class="error-message" name="linkDescription" />
                <error-message class="error-message" name="externalLink" />
              </div>
            </div>
            <div class="w-1/12 mb-2 flex justify-center items-center cursor-pointer">
              <font-awesome-icon
                icon="fa-plus"
        ...
        <!-- <field-label class="w-full md:w-1/2" name="Enable Partial Payment" v-if="isControlledMoneyAvailable">
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" v-model="state.enableParticalPayment" class="sr-only peer">
            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
            <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300" v-if="state.enableParticalPayment">Enabled</span>
            <span class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300" v-else>Disabled</span>
          </label>
        </field-label>

        <field-label class="w-full md:w-1/2" name="Amount to Client" :isRequired="true" v-if="state.enableParticalPayment">
          <v-field
            placeholder="Amount to Client"
            name="amountToControlledMoneyAccount"
            type="number"
            rules="required"
          />
          <error-message class="error-message" name="amountToControlledMoneyAccount" />
        </field-label> -->
      </alp-form-container>
    </modal-form>
  </modal>
...
import AvailableTrustAccountForMatterSelector from "@/components/inputs/selectors/AvailableTrustAccountForMatterSelector.vue";
...
import {
  TrustAccountDto,
  TrustAccountListDto,
  TrustTransactionRequestInput,
} from "@/network/dtos/trust-dto";
...
interface CreateRequestValues {
  ...
  // Controller Money Trust Account Partial Payment
  enableParticalPayment?: boolean;
  amountToControlledMoneyAccount?: number;
}
...
const state = reactive({
  ...
  linkDescriptions: [] as string[],
  externalLink: "",
  linkDescription: "",
  linkDescPair: [] as string[],
  supportDocumentLinks: [] as ResourceUrlDto[],
  colorChange: "h-8 w-8 text-gray-500",
  buttonLoading: false,
  trustAccount: null as TrustAccountListDto | null,
  enableParticalPayment: false,
  date: toDateTime(new Date())?.toUTC(),
});

const { toDropdownOptions: toTransactionTypeOptions } = useEnum(TransactionType);
const transactionTypes = toTransactionTypeOptions();

const { toDropdownOptions: toTransactionMethodOptions } = useEnum(TransactionMethod);
...
const { toDropdownOptions: toWithdrawalPurposeOptions } = useEnum(WithdrawalPurpose);
const withdrawalPurposes = toWithdrawalPurposeOptions();

const { toDropdownOptions: toPaymentDirectionOptions } = useEnum(PaymentDirection);
const paymentDirections = toPaymentDirectionOptions();

function createRequest(values: CreateRequestValues) {
  if (values != undefined) {
    if (props.isControlledMoneyAvailable) {
      if (state.enableParticalPayment && values.amountToControlledMoneyAccount != null) {
        if (values.amount - values.amountToControlledMoneyAccount <= 0) {
          alert(
            "Amount to Controlled Money Trust Account cannot be greater than total amount"
          );
          return;
        }
      }
    ...
    request.init(values);
    request.transactionMethod = values.transactionMethod;
    request.transferToMatterId = values.transferToMatter?.id;
    request.supportDocumentLinks = state.supportDocumentLinks;
    request.trustAccountId = state.trustAccount?.id;

    request.enableParticalPayment = state.enableParticalPayment;
    request.amountToControlledMoneyAccount = values.amountToControlledMoneyAccount;
    request.transactionDate = toDateTime(values.transactionDate)?.toUTC();
    store
      .dispatch(TrustStore.actions.CREATE_MATTER_TRUST_REQUEST, {
        id: props.id,
        request,
      })
      .then(() => {
...
function addExternalLink() {
  ...
  if (!isEmptyOrSpaces(getDesc) && !isEmptyOrSpaces(getLink) && validURL(getLink)) {
