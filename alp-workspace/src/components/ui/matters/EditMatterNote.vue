  ...
  <modal @close="$emit('close')" :headingTitle="'Update Note'">
    <div class="flex flex-col rounded-lg bg-white min-h-0">
      <div class="flex flex-col pb-3 px-6">
        ...
        <span class="flex justify-end space-x-2 py-2 flex-wrap gap-2 sm:gap-0">
...
import {
  MatterNoteServiceProxy
} from "@/network/matters-service-proxies";
  ...
  setup(props, { emit }) {
    ...
    function updateNote() {
      if (state.note.message) {
        if (state.fileName.length > 0) {
          ...
          store
            .dispatch(MatterStore.actions.ADDUPDATE_MATTER_NOTE_UPLOAD, {
              id: props.id,
          ...
          store
            .dispatch(MatterStore.actions.ADDUPDATE_MATTER_NOTE, {
              id: props.id,
              attachments: state.removeAttachmentId,
              note: new NoteInput({
                message: state.note.message,
    ...
    function uploadDocuments(event: Event) {
      ...
      for (var i = 0; i < Fileslength; i++) {
        state.uploadFile.push(target?.files[i]);
      }
      const targetFile = Array.from(target?.files || []) as any;
