  ...
  <modal @close="$emit('close')" :headingTitle ="'Update Note'">
    <div class="flex flex-col rounded-lg bg-white min-h-0">
      <div class="flex flex-col pb-3 px-6">
        <editor editor-container-class="max-h-full" v-model="state.note.message" />
        <div class="home w-full">
          <UploadDocument @drop.prevent="drop" />
        </div>
        <span class="flex flex-wrap gap-2 mt-1">
        <span class="bg-gray-100 text-sm flex gap-2 items-center px-2 py-1 rounded-lg" v-for="(item,index) in state.note.document"
          :key="item.id">
      ...
      />
          ...
          <span class="bg-gray-100 text-sm flex gap-2 items-center px-2 py-1 rounded-lg" v-for="(item,index) in state.fileName"
          :key="item">
...
import { ContactNoteServiceProxy } from "@/network/crm-service-proxies";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      new ContactNoteServiceProxy()
        .getContactNoteById(props.id, props.noteid)
        .then((result: NoteDto) => {
            state.note = result;
        });
    });
    ...
    function updateNote() {
      if (state.note.message) {
        if(state.fileName.length > 0){
          fireSuccessToast(
        "Please wait while we are uploading files ..."
      );
      store
          .dispatch(ContactStore.actions.ADDUPDATE_CONTACT_NOTE_UPLOAD, {
            id: props.id,
        ...
        {
        store
          .dispatch(ContactStore.actions.ADDUPDATE_CONTACT_NOTE, {
            id: props.id,
            attachments: state.removeAttachmentId,
            note: new NoteInput({
              message: state.note.message,
