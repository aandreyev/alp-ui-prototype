<template>
  <modal @close="$emit('close')" :headingTitle="'Deposit Funds in Trust'">
    <modal-form
      @cancel="$emit('close')"
    ...
    >
      <alp-form-container>
        ...
        <field-label class="w-full md:w-1/2" name="Amount to Controlled Money Account" :isRequired="true" v-if="state.enableParticalPayment">
          <v-field
            placeholder="Amount to Controlled Money Account"
...
function createRequest(values: CreateRequestValues) {
  if (values != undefined) {
    if (props.isControlledMoneyAvailable) {
      if (
        state.enableParticalPayment &&
        values.amountToControlledMoneyAccount != null
      ) {
    ...
    request.amountToControlledMoneyAccount =
      values.amountToControlledMoneyAccount;
