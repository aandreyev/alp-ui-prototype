<template>
  <modal @close="$emit('close')" :headingTitle="'Transfer Funds'">
    <modal-form
      @cancel="$emit('close')"
...
function createRequest(values: CreateRequestValues) {
  if (values != undefined && props.id != values.transferToMatter?.id) {
    ...
    // throw error
    fireErrorToast(
      "The request is invalid, or the funds are being transferred to the same matter."
    );
  }
}
