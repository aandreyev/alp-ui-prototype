<template>
  <modal @close="$emit('close')" :headingTitle="'Add Safe Storage For Contact'">
    <modal-form
      @cancel="$emit('close')"
      @submit="createContactSafeStorage"
      v-slot="{ values }"
      :initialValues="{
...
export default defineComponent({
  ...
  components: {
    ...
    ErrorMessage,
    OfficeSelectorField,
    SafeStorageSectionSelectorField,
    SafeStorageDocumentTypeSelectorField,
    Switch,
    Label
  },
  setup(props, { emit }) {
    ...
    function createContactSafeStorage(values: any) {
      (values.dateOfDocuments = toDateTime(values.dateOfDocuments)?.toUTC()),
        store
          .dispatch(CommonStore.actions.CREATE_CONTACT_SAFE_STORAGE_DOCUMENT, {
            ...values,
    ...
    return {
      ...
      StatusTypes,
      toDateTime,
      chargingClauseOptions,
      ChargingClauseTypesTypes
    };
  }
});
</script>
