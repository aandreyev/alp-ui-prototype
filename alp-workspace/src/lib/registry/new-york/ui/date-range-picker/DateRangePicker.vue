...
function GetFilterState() {
  let filterData = filterState.getItem(enumFilterState.TimeEntriesDatePicker);
...
function SetFilterState() {
  ...
  filterState.setItem(enumFilterState.TimeEntriesDatePicker, filterStateData);
}
watch(
  [() => GetFilterState()],
  () => {
    ...
    <shad-popover v-else>
      <shad-popover-trigger as-child>
        <shad-button
          ...
          :class="
            cn(
              'w-[280px] justify-start text-left font-normal h-8 px-3',
              !date && 'text-muted-foreground'
            )
          "
        >
